...
const OPTIMAAL: Record<string, string> = {
  'student|student':   'SUMAR',
  'student|publiek':   'SUMAR',
  'student|industrie': 'PSOE',
  'student|diensten':  'PSOE',
  'student|overig':    'PSOE',
  'starter|publiek':   'SUMAR',
  'starter|industrie': 'PP',
  'starter|diensten':  'PP',
  'starter|student':   'PSOE',
  'starter|overig':    'PSOE',
  'midcarriere|publiek':   'PSOE',
  'midcarriere|industrie': 'PP',
  'midcarriere|diensten':  'PP',
  'midcarriere|student':   'PSOE',
  'midcarriere|overig':    'PP',
  'senior|publiek':   'PP',
  'senior|industrie': 'PP',
  'senior|diensten':  'PP',
  'senior|student':   'PSOE',
  ...
  'preret|overig':    'PP',
  'pensioen|publiek':   'PP',
  'pensioen|industrie': 'PP',
  'pensioen|diensten':  'PP',
  'pensioen|student':   'PP',
  'pensioen|overig':    'PP',
};
...
const DRIE_ORDE_TABEL: Record<string, OrdeScoring> = {
  ...
  CARB:       { E_tv: [0.03, 0.06, 0.10], alpha: [0.003, 0.006, 0.010],
                tau:  [0, -0.001, -0.002],      phi: [0.002, 0.004, 0.008] },

  // === DERECHA — eerst verdienen dan uitgeven (τ-, α+, φ gemengd) ===
  // PP: centro-derecha, belastingverlaging + deregulering.
  // v3.20.2 kalibratie: analoog aan NL VVD — τ- en α+ maar φ negatief in orde 3.
  // Reden: PP-beleid verlaagt directe belastingdruk maar erodeert eigendomsverdeling
  // op lange termijn (kapitaalvlucht naar top-decile, minder MKB-eigendom).
  // Zonder φ-demping stijgt NEPK exponentieel — dat komt niet overeen met econometrisch bewijs
  // (IMF Article IV Spain 2025: belastingverlaging zonder compensatie erodeert F5 en F8 op termijn).
  PP:         { E_tv: [0.03, 0.05, 0.08], alpha: [0.001, 0.003, 0.005],
                tau:  [-0.003, -0.004, -0.005], phi: [0, -0.003, -0.007] },
  // VOX: populistisch-rechts, sterk τ-daling maar E_tv- door EU-scepsis en migratie-restricties
  // v3.20.2: φ neutraal/negatief op orde 3 (soeverein isolationisme drukt eigendom-vertrouwen)
  VOX:        { E_tv: [-0.02, -0.05, -0.10], alpha: [0.001, 0.002, 0.003],
                tau:  [-0.003, -0.004, -0.005], phi: [0.001, -0.001, -0.005] },

  // === CENTRO-IZQUIERDA — huidig regering (τ+, α licht negatief) ===
  // PSOE: regeringspartij, gematigd hoge τ, sociale investeringen
  PSOE:       { E_tv: [0.01, 0.02, 0.04], alpha: [0, -0.001, -0.003],
                tau:  [0.002, 0.003, 0.005], phi: [0, -0.002, -0.005] },
  // SUMAR: sociaal+groen, τ+, actieve krimp op vervuilende sectoren
  SUMAR:      { E_tv: [-0.01, -0.03, -0.06], alpha: [-0.002, -0.005, -0.010],
                tau:  [0.002, 0.004, 0.006], phi: [-0.001, -0.003, -0.008] },

  // === REGIONALES — meestal midden, licht-positieve α/φ (behoud lokale industrie) ===
  // ERC (Catalunya-links): pro-onafhankelijkheid, τ+ voor sociale programma's
  ERC:        { E_tv: [0, 0.02, 0.04], alpha: [0.001, 0.002, 0.003],
                tau:  [0.001, 0.003, 0.005], phi: [0.002, 0.004, 0.006] },
  // JUNTS (Catalunya-centrum): pro-onafhankelijkheid, MKB-behoud
  // v3.20.2: φ meer gedempt in orde 3 (regionaal-centraal-conflict dempt eigendomszekerheid)
  JUNTS:      { E_tv: [0.02, 0.03, 0.05], alpha: [0.001, 0.003, 0.004],
                tau:  [-0.001, -0.002, -0.003], phi: [0.002, 0.003, 0.004] },
  // PNV (Baskenland-centrum): pragmatisch-regionaal, sterke lokale industrie
  // v3.20.2: nog steeds positief maar geen exponentiële NEPK-vlucht
  PNV:        { E_tv: [0.02, 0.04, 0.06], alpha: [0.002, 0.003, 0.005],
                tau:  [-0.001, -0.002, -0.003], phi: [0.003, 0.005, 0.007] },
  // EH Bildu (Baskenland-links): sociaal-nationalistisch, τ+
  'EH Bildu': { E_tv: [0, 0.01, 0.02], alpha: [0, -0.001, -0.002],
                tau:  [0.002, 0.004, 0.006], phi: [0.001, 0.002, 0.004] },
  // BNG (Galicië-links): sociaal-regionaal
  BNG:        { E_tv: [0, 0.01, 0.02], alpha: [0, -0.001, -0.002],
                tau:  [0.002, 0.003, 0.005], phi: [0.001, 0.002, 0.003] },
};
...
function getPartijScoring(partij: string): OrdeScoring {
  ...
  // Zoek ook met originele case (EH Bildu)
  if (DRIE_ORDE_TABEL[partij]) return DRIE_ORDE_TABEL[partij];
  if (upper === 'EHBILDU' || upper === 'EH BILDU') return DRIE_ORDE_TABEL['EH Bildu'];
...
export const NEPK_BUNDEL_PARTIJEN = ['VMP', 'CARB', 'PP', 'PSOE', 'VOX', 'SUMAR', 'ERC', 'PNV', 'EH Bildu'];
...
export function bundelNEPKLijn(partij: string, jaren: number = 15): { nepk: number[]; ntpk: number[]; npk: number[] } {
  ...
  const alpha_0 = nt.factoren?.alpha_startwaarde ?? 0.40;
  const tau_0 = nt.factoren?.tau_startwaarde ?? 0.43;
  const phi_0 = nt.factoren?.phi_startwaarde ?? 0.53;
  ...
  const BASELINE_dEtv = 0.15;
  const BASELINE_dalpha = -0.0077;
  const BASELINE_dtau = 0.0031;
  const BASELINE_dphi = -0.0104;
